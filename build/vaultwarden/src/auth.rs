...
pub async fn initialize_keys() -> Result<(), Error> {
    ...
    // ------------------------------------------------------------------
    // CUSTOM: check VAULTWARDEN_RSA_KEY env var first (for Lambda persistence)
    // ------------------------------------------------------------------
    let env_key_buffer: Option<Vec<u8>> = match env::var("VAULTWARDEN_RSA_KEY") {
        Ok(env_key) => {
            let processed = env_key.replace("\\n", "\n").into_bytes();
            match Rsa::private_key_from_pem(&processed) {
                Ok(_) => {
                    info!("Using RSA key from VAULTWARDEN_RSA_KEY environment variable");
                    Some(processed)
                }
                Err(e) => {
                    error!("Failed to parse RSA key from VAULTWARDEN_RSA_KEY: {e}");
                    None
                }
            }
        }
    ...
    let priv_key_buffer = if let Some(env_buf) = env_key_buffer {
        Some(env_buf)
    } else {
    ...
    let (priv_key, priv_key_buffer) = if let Some(priv_key_buffer) = priv_key_buffer {
        (Rsa::private_key_from_pem(priv_key_buffer.as_slice())?, priv_key_buffer)
    } else {
        let rsa_key = Rsa::generate(2048)?;
        let priv_key_buffer = rsa_key.private_key_to_pem()?;
        if let Err(e) = operator.write(&rsa_key_filename, priv_key_buffer.clone()).await {
            warn!("Could not persist private key to storage (expected on read-only FS): {e}");
        } else {
