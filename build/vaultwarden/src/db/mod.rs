...
impl DbConnManager {
    ...
    fn establish_connection(&self) -> Result<DbConnInner, diesel::r2d2::Error> {
        match DbConnType::from_url(&self.database_url) {
            ...
            #[cfg(postgresql)]
            Ok(DbConnType::Postgresql) => match diesel::pg::PgConnection::establish(&self.database_url) {
                Ok(conn) => Ok(DbConnInner::Postgresql(conn)),
                Err(e) => {
                    println!("Diesel connection error (Debug): {:#?}", e);
                    println!("Diesel connection error (Display): {}", e);
                    return Err(e.into());
                }
            },
            #[cfg(sqlite)]
